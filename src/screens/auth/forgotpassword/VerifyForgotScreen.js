...
const VerifyForgotScreen = ({ navigation, route }) => {
  ...
  const resendCode = async () => {
    try {
      const apiUrl = `http://192.168.43.250:8080/resend-reset?token=${apiToken}`;
  ...
  const sendOTP = async () => {
    try {
      ...
      const apiUrl = `http://192.168.43.250:8080/verify-reset?token=${apiToken}`;
      ...
      if (response.ok) {
        ...
        if (response.status === 200) {
          ...
          const apiTokenFromResponse  = responseData?.data?.token;
          const email  = responseData?.data?.email;

          Alert.alert(responseData?.message);
          // Proceed with any additional logic or navigation
          navigation.navigate('NewPassword',  { apiToken: apiTokenFromResponse, email:email });
        } else {
