...
const TapScreen = ({ navigation }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      const response = await axios.post('http://192.168.43.250:8080/api/v1/logout', null, {
        headers: {
  ...
  const handleTapIn = async () => {
    try {
      ...
      const response = await axios.post(
        'http://192.168.43.250:8080/api/v1/attendance',
        { tap_in: currentTime },
        {
          headers: {
  ...
  const handleTapOut = async () => {
    try {
      ...
      const response = await axios.put(
        `http://192.168.43.250:8080/api/v1/attendance/${id}`,
        { tap_out: currentTime },
        {
          headers: {
            Authorization: `Bearer ${await AsyncStorage.getItem('jwtToken')}`,
